...
`Derives` — now parameterized by `Strategy`, mutual with `evalArgsPar`
(the `.callByValuePar` argument-evaluation helper). Same project-wide fuel
convention: `none` means "out of fuel" and nothing else; `some .fail` is a
legitimate semantic outcome.
