...
- **呼び出し箇所での Prop 引数**（M-PEG-5 で判明、v6 以降の課題）: `sigParams`
  （`Lens/Translate.lean`）は関数**定義**の Prop 引数（例: `h : acyclicB g = true`
  のような整形式性の証明witness）を消去できるが、その関数を**呼び出す側**で
  具体的な証明項（`theorem`）を実引数として渡すと、抽出器は「theorem leaked
  into executable code」で fail-loud に拒否する——呼び出し箇所での Prop 引数消去
  は未実装。`MExp.expand`/`MGrammar.expandGrammar`（M-PEG-5）自体は等式補題ルート
  経由で抽出できるが、`Corpus.lean` のような呼び出し側から証明項を渡す形の
  差分ケースは追加できず、実機（reference Scala）との突き合わせは自動 corpus diff
  ではなく一回限りの手動検証で代替した
