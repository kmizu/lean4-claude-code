...
- **Prop 引数**（M-PEG-5 で追加）: 整形式性・停止性の証明witness
  （例: `h : acyclicB g = true`）を取る関数の**定義**（`sigParams`）と
  **呼び出し箇所**（`transApp`／`transMatcher`）の両方で、Prop 型の引数を
  Scala の `Unit` 引数・`()` リテラルへ消去する。名前付き match
  （`match h : e with ...`）が各 alt に注入する等式証明の引数も同じ経路で消去
  される。証明項そのものの値は使われない（proof irrelevance）ので、どんな
  Prop 項が渡されていても安全に消去できる。
