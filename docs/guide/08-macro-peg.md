...
前節の「抽出器のもう一つの発見」を書いた直後、同じセッション内でこの壁を
実際に塞ぎました。原因を辿ると、`Lens/Translate.lean`には Prop 引数を扱う
場所が3箇所ありました：

1. `sigParams`——関数**定義**側のパラメータ一覧を作る箇所。ここは既に
   Prop 引数を`err`で拒否していましたが、`MExp.expand`のような等式補題ルート
   経由の関数はこの拒否を素通りしていました（`err`が発火する前に、証明項の
   翻訳の方で先に「theorem leaked」エラーが出ていたからです）。
2. `transApp`——**呼び出し箇所**で実引数を翻訳する箇所。ここには Prop 引数を
   飛ばす仕組みが一切なく、証明項をそのまま`transTerm`に渡そうとして
   「theorem leaked into executable code」で落ちていました。
3. `transMatcher`——ネストした`match`を分解する箇所。`match h : e with ...`
   という名前付き match は、各 alt に「判別式とパターンが等しい」という
   証明を追加の引数として注入します。これも Prop なので、素朴に翻訳しようと
   すると「non-variable in match-equation RHS」で落ちました。

直した方針はどこも同じです：**Prop 型の値は proof irrelevance によって
実行時には何の情報も運ばない**ので、安全に`Unit`型・`()`リテラルへ消去して
よい、というものです。

- `sigParams`は Prop 引数を**拒否せず**、`Unit`型のパラメータとして
  温存するようにしました。ここで「位置を潰さない」（消去はするが、その
  パラメータ自体は残す）ことが肝心です——`transDefViaEqns`のパターン位置の
  インデックス計算（`sigNames[i - nT]!`のような式）は、全パラメータの個数と
  順序が保たれていることに依存しているので、もし Prop 位置を「詰めて」
  しまうとこのインデックスが総崩れになります。位置を保ったまま型だけ
  `Unit`にする、というのが最小のリスクで済む修正でした。
- `transApp`は、呼び出し先の宣言された型を`sigParams`と同じ`Meta.isProp`
  で位置ごとに調べ、Prop 位置の実引数は翻訳せずに`()`リテラルへ差し替える
  ようにしました。
- `transMatcher`は、alt の RHS に現れる引数が「パターン変数」でも
  「`Unit.unit`」でもない場合、それでも型が Prop なら消去してよい、という
  一般化を加えました。名前付き match が注入する証明引数は、まさにこの
  ケースに当てはまります。

この3箇所を直したことで確認できたこと：既存27ルートはどれも Prop 引数を
持っていなかったので退行のリスクがなく、`make verify`で既存60＋318＋20
ケース全件が無退行であることをまず確認しました。そのうえで
`Corpus.lean`に`MExp.expand`を実際に呼び出す差分ケース（corpus ID
`340`/`341`）を追加し、Lens で自動抽出した Scala コードと Lean ネイティブ
実行・golden の3方一致（計22ケース）を達成しました。手動の`sbt console`
検証に頼る必要はもうありません。

生成された Scala コードはちょうど狙い通りの形になりました：
...
`h`は`Unit`型のパラメータとして残り、呼び出し側では実際の証明項の代わりに
`()`が渡ります——Lean の型システムが保証した「この文法は非循環」という
事実は、Scala のコードには一切現れず、ただ静かに消えています。
